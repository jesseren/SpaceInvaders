...
public class LaserEnemy extends GameObject{
    ...
    public LaserEnemy(float x, float y, ID id, Game game, Image[] image, int width, int height, Handler handler){
    ...
    public void tick(){
        if(health == 0) {
            for(int i = 0; i < handler.object.size(); i++) {
                ...
                if (tempObject.getId() == ID.EnemyLaser && tempObject.getBounds().intersects(this.getBounds())) {
        ...
        if(counter <= 30){
            ...
            if(counter == 60) {
                counter = 0;
                bulletCount++;
                if(bulletCount == 4 && !end) {
                    handler.add(new EnemyLaser(x+8,y+10,ID.EnemyLaser,game,game.images.get(ID.EnemyLaser),gameWidth,gameHeight,handler));
                    bulletCount = 0;
                }
            }
        }
